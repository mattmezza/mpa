...
def connect(provider: dict) -> caldav.Calendar:
    """Connect to a CalDAV server and return the principal's default calendar."""
    ...
    # If the URL points directly to a calendar, use it; otherwise use the default
    calendars = principal.calendars()
    if not calendars:
        print("Error: no calendars found for this account", file=sys.stderr)
        sys.exit(1)

    # Return first calendar (most providers expose the primary one first)
    return calendars[0]
