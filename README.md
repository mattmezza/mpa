...
- **Coding harness** — Optional file tools (`read_file`, `write_file`, `edit_file`, `list_dir`, `grep`, `run_command_in_dir`) that let the agent work on a real codebase directly, all confined to one configurable workspace directory (paths escaping it via `..` or symlink are blocked). Reads are pre-approved; writes and commands ask for approval each time (off by default)
