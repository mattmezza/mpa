...
1. Make sure a machine key exists (`MPA_MASTER_KEY` set, or click **Generate & save key**
   on the Secrets tab / wizard — it writes `data/master.key`).
2. Click **Migrate N credential(s) into the vault** on the Secrets tab (or **Move … into
   the vault** in the wizard).
