...
- **Bot-per-agent** (opt-in) — give a agent its own bot token and it becomes a separate Telegram contact, running alongside the default bot in the same process; see below
...
- **tags every speaker** — each message a bot sees is recorded with a `[from <author>]`
  prefix (`(bot)` for bots), so a agent is never confused about who said what in the
