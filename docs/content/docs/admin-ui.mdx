...
- **Assistant** — the default identity (agent name, character & tone), used when no agent is active
- **Agents** — create, edit, and switch swappable agent identities (see below)
...
Manage swappable agent identities — each with its own character, skill allowlist, tool scope, voice, and secret scope. Cards show every agent and the active one; **Use** activates one (applied live to new turns) or revert to **Default** (the Assistant-tab identity). The editor offers a **guided form** (identity, agent name, voice with a **Preview** button and Edge/Kokoro suggestions, skill + tool checkboxes, secret scope) and a **raw markdown** toggle for power users. Six starter agents ship seeded. The page is responsive at phone width. See [Agents](/docs/agents) for the full model.
...
- Create new jobs (optionally scoped to a agent for `agent`/`agent_silent`/`subagent` types)
- Pause/resume jobs
- Delete jobs
...
/ login secrets — a **agent × secret access matrix** to grant a agent use of a secret, plus
