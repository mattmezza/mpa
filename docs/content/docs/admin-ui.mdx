...
Manage the default agent and swappable agent identities on one tab (#115). The **Default agent** — name, character & tone, voice, and account bindings, used when no other agent is active — sits at the top; below it, cards show every custom agent and the active one, each with its own character, skill allowlist, tool scope, voice, and secret scope. **Use** activates one (applied live to new turns) or revert to **Default** (the default agent above). The editor offers a **guided form** (identity, agent name, voice with a **Preview** button and Edge/Kokoro suggestions, skill + tool checkboxes, secret scope) and a **raw markdown** toggle for power users. Six starter agents ship seeded. The page is responsive at phone width. See [Agents](/docs/agents) for the full model.
