...
- A agent-specific rule **overrides** a Default rule for the same action — so a `coach` agent can be allowed to `git push` while `finance` still has to ask.
- Any action a agent has **no** rule for resolves through the Default ruleset (which is where the shipped defaults live). Agents without explicit rules behave exactly as before.
- The hard `NEVER` rails and the wildcard/shell-injection guards apply within whichever ruleset is in effect.

When the agent runs under a agent, its approvals are learned into that agent's scope only — approving a command for `coach` never silently widens it for everyone.
