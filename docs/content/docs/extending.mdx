...
Per-persona: in the **Personae** editor under *Tool identities*, choose each persona's
**GitHub identity** explicitly — **Personal Access Token** (its own token or a reused vault
secret) or **GitHub App**. In App mode a persona can carry its **own** app (App ID +
Installation ID + a vault-referenced PEM) and run as a distinct `<app>[bot]`, or leave those
blank to use the system-wide App bot. So you can run **multiple apps — one per persona** —
freely mixed with PAT personae; a persona never borrows the owner's PAT. Optionally add an
