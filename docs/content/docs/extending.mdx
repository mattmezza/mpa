...
Enable `gh` under **Settings → Tools → GitHub CLI (gh)** and pick an auth mode:
...
Per-persona: give a persona its own GitHub identity (its own PAT, a shared vault secret, or
the App bot) in the **Personae** editor under *Tool identities*, and optionally an **allowed
repos** list — a best-effort `--repo` gate on top of the app installation's own repo
selection (which is the real, server-enforced boundary). Subagents inherit that repo scope
and can only narrow it, never widen it.
