...
You can also set the backend and default voice from the admin UI (**Assistant → Voice**), and pick a per-agent voice in the agent editor. Both have a **Preview** box: type a few words, optionally pick a **language**, and hear the voice before saving. Because the voice (timbre) and the pronunciation language are independent in Kokoro, any voice can read any language — e.g. an Italian voice reading English.
...
A agent can speak in its own voice — set `voice:` in the agent's frontmatter
