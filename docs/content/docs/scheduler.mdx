...
Each row shows the **agent** the job runs as (or `—` for the default identity), and a dropdown narrows the list to a single agent. An `agent`, `agent_silent`, or `subagent` job can be scoped to a agent at create/edit time so it runs under that identity.
...
chat — so a reminder set by a agent in a group chat is posted by that agent in
