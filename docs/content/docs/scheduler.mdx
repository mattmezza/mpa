...
| **subagent** | Task run by a [subagent](/docs/subagents) under a `agent` | "Draft my weekly review" |
...
| `channel` | No | Delivery channel for agent jobs: `telegram` (default agent's bot) or `telegram:<agent>` (a specific agent's bot) |
...
Each row shows the **agent** the job runs as (or `—` for the default identity), and a dropdown narrows the list to a single agent. An `agent`, `agent_silent`, or `subagent` job can be scoped to an agent at create/edit time so it runs under that identity.
...
job fires it runs as the agent that created it and is delivered back to the same
chat — so a reminder set by an agent in a group chat is posted by that agent in
